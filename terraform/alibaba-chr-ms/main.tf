...
# Whitelist SSH (Custom Hardened Port 7822)
resource "alicloud_security_group_rule" "peering_ingress_ssh" {
  ...
  description       = "SSH management on custom port ${var.ssh_port}"
}

# Whitelist Winbox (Custom Hardened Port 7881)
resource "alicloud_security_group_rule" "peering_ingress_winbox" {
  ...
  security_group_id = alicloud_security_group.peering_sg.id
  cidr_ip           = var.admin_ip_cidr
  description       = "Winbox management on custom port ${var.winbox_port}"
}

# Allow IPsec IKEv2 (UDP 500) from Alibaba NextOps CHR Public IP
resource "alicloud_security_group_rule" "peering_ingress_nextops_ipsec_ike" {
  type              = "ingress"
  ip_protocol       = "udp"
  nic_type          = "intranet"
  policy            = "accept"
  ...
  port_range        = "500/500"
  security_group_id = alicloud_security_group.peering_sg.id
  cidr_ip           = "${var.nextops_chr_public_ip}/32"
  description       = "IPsec IKEv2 UDP 500 from NextOps CHR Public IP"
}

# Allow IPsec NAT-Traversal (UDP 4500) from Alibaba NextOps CHR Public IP
resource "alicloud_security_group_rule" "peering_ingress_nextops_ipsec_natt" {
  type              = "ingress"
  ip_protocol       = "udp"
  nic_type          = "intranet"
  policy            = "accept"
  ...
  security_group_id = alicloud_security_group.peering_sg.id
  cidr_ip           = "${var.nextops_chr_public_ip}/32"
  description       = "IPsec NAT-T UDP 4500 from NextOps CHR Public IP"
}

# Allow GRE Protocol from Alibaba NextOps CHR Public IP
resource "alicloud_security_group_rule" "peering_ingress_nextops_gre" {
  type              = "ingress"
  ip_protocol       = "gre"
  nic_type          = "intranet"
  policy            = "accept"
  ...
  security_group_id = alicloud_security_group.peering_sg.id
  cidr_ip           = "${var.nextops_chr_public_ip}/32"
  description       = "GRE Tunnel from NextOps CHR Public IP"
}

# Allow BGP (TCP 179) from Alibaba NextOps CHR Public IP
resource "alicloud_security_group_rule" "peering_ingress_nextops_bgp" {
  type              = "ingress"
  ip_protocol       = "tcp"
  nic_type          = "intranet"
  policy            = "accept"
  ...
  security_group_id = alicloud_security_group.peering_sg.id
  cidr_ip           = "${var.nextops_chr_public_ip}/32"
  description       = "BGP TCP 179 from NextOps CHR Public IP"
}
...
# Allow all internal ManagedService VPC traffic on the private interface
resource "alicloud_security_group_rule" "private_ingress_internal_vpc" {
  ...
  description       = "Allow internal ManagedService VPC traffic on private ENI"
}

# Allow NextOps VPC traffic on private ENI
resource "alicloud_security_group_rule" "private_ingress_nextops_vpc" {
  type              = "ingress"
  ip_protocol       = "all"
  nic_type          = "intranet"
  policy            = "accept"
  ...
  port_range        = "-1/-1"
  security_group_id = alicloud_security_group.private_sg.id
  cidr_ip           = "10.151.0.0/18"
  description       = "Allow NextOps VPC traffic on private ENI"
}

# Allow GCP Shared VPC traffic on private ENI
...
# Allow Azure VNet traffic on private ENI
resource "alicloud_security_group_rule" "private_ingress_azure_vnet" {
  ...
  port_range        = "-1/-1"
  security_group_id = alicloud_security_group.private_sg.id
  cidr_ip           = "10.126.0.0/18"
  description       = "Allow Azure VNet traffic on private ENI"
}

# Allow AWS Singapore VPC traffic on private ENI
resource "alicloud_security_group_rule" "private_ingress_aws_sin_vpc" {
  type              = "ingress"
  ip_protocol       = "all"
  nic_type          = "intranet"
  policy            = "accept"
  ...
  port_range        = "-1/-1"
  security_group_id = alicloud_security_group.private_sg.id
  cidr_ip           = "10.29.0.0/18"
  description       = "Allow AWS Singapore VPC traffic on private ENI"
}

# Allow AWS Jakarta Managed Service VPC traffic on private ENI
resource "alicloud_security_group_rule" "private_ingress_aws_jkt_vpc" {
  type              = "ingress"
  ip_protocol       = "all"
  nic_type          = "intranet"
  policy            = "accept"
  ...
  description       = "Allow AWS Jakarta Managed Service VPC traffic on private ENI"
}
